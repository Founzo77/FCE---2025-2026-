    ...
    in float3 ks, in float shininess, in RayPayload oldPayload)
{
    ...
    for (int i = 0; i < nbLights; i++)
    {
        ...
        shadowRayPayloadSetNbBounds(payload, mainRayPayloadGetNbBounds(oldPayload) + 1);
    ...
    in BuiltInTriangleIntersectionAttributes attr)
{
    ...
    if(nbBounds < MAX_RAY_NB_BOUNDS)
    {
        ...
        if(computeRadiance(reflectanceWeight) >= MIN_REFLECTANCE_WEIGHT_FOR_BOUND)
        {
            ...
            finalColor += mainRayPayloadGetColor(secondaryPayload);
        }
...
void surfaceSampleBsdf(in Intersection intersection, inout LightSample lightSample)
{
    ...
    float3 wi = intersection.m_worldWi; // vers caméra
    float3 wo = lightSample.m_worldWo; // vers lumière

    float3 radianceLight = lightSample.m_radiance;

    // DIFFUSE
    float3 diffuse = albedo * radianceLight;

    // SPECULAR (Phong)
    float3 R = reflect(-wo, normal);
    float RdotWi = max(dot(R, wi), 0.0f);

    float3 specular = material.m_ks * pow(RdotWi, material.m_shininess) * radianceLight;

    lightSample.m_surfaceColor = diffuse + specular;
}

void surfaceGetWeight(in Intersection intersection, in float3 wi, in float3 wo, out float3 weight)
{
    // récupérer material
    SubMeshData subMeshData = g_subMeshData;
    Material material =
    ...
    if(isReflectance(wi, intersection.m_tbnShadingNormal, wo)) // Reflexion
    {
        weight = material.m_reflectance;
    }
    else // Transmission
    {
        weight = material.m_transmittance;
    }
}
    ...
    const uint surfaceLobeType)
{
    ...
    float cosTheta = max(dot(normal, tbnWo), 0.0f);

    color = float3(0, 0, 0);

    if(isReflectance(tbnWi, tbnWo))
    {
        if(hasType(surfaceLobeType, SURFACE_LOBE_DIFFUSE_REFLECTANCE))
        {
            float3 R = reflect(-tbnWo, normal);
            float RdotV = max(dot(R, tbnWi), 0.0f);

            float3 specular = material.m_ks * pow(RdotV, material.m_shininess);

            color += albedo * cosTheta + specular;
        }
        
        if(hasType(surfaceLobeType, SURFACE_LOBE_DIRAC_REFLECTANCE))
        {
    ...
    {
        if(hasType(surfaceLobeType, SURFACE_LOBE_DIRAC_TRANSMITTANCE))
        {
    ...
    out float3 tbnWo, out float pdf, const uint surfaceLobeType)
{
    ...
    else if(hasType(surfaceLobeType, SURFACE_LOBE_DIFFUSE_TRANSMITTANCE))
    {
        float2 rand = randomFloat2(rayPayload);
        tbnWo = cosineSampleHemisphere(rand);
        pdf = tbnWo.z / PI;
    ...
    else if(hasType(surfaceLobeType, SURFACE_LOBE_DIRAC_REFLECTANCE))
    {
        float3 normal = intersection.m_tbnShadingNormal;
        if(tbnWi.z < 0)
            normal = -normal;
    ...
    else if(hasType(surfaceLobeType, SURFACE_LOBE_DIRAC_TRANSMITTANCE))
    {
        pdf = 1;
        tbnWo = -tbnWi;
    }
...
[shader("closesthit")]
void PhongClosestHit(inout RayPayload payload, in BuiltInTriangleIntersectionAttributes attr)
{
